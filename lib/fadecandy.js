...
(function () {
    ...
    fadecandy.enumerateDeviceLeds = function(devices) {
        // Return a list of objects describing the potential LEDs we may find on a list of FC devices.

        var results = [];

        for (var i = 0; i < devices.length; i++) {
            var device = devices[i].serial;

            for (var j = 0; j < 512; j++) {
                results.push({
                    device: device,
                    index: j,
                    stripIndex: (j / fadecandy.LEDS_PER_STRIP)|0,
                    stripPosition: (j % fadecandy.LEDS_PER_STRIP)|0,
                    string: sprintf('%s-%03d', device, j),
                });
            }
        }
